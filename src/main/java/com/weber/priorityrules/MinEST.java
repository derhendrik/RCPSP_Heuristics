...
public class MinEST extends AbstractPriorityRule {
    ...
    public List<Node> generate_activity_list(Project project, Scheduler scheduler) {
        List<Node> activity_list = project.nodes.stream().sorted(Comparator.comparing(n -> n.es)).collect(Collectors.toList());
