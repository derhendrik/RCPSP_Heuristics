...
public class MaxLPT extends AbstractPriorityRule {
    public MaxLPT() {
        this.name = "LPT";
        this.mm_type = "max";
    }
    ...
    public List<Node> generate_activity_list(Project project, Scheduler scheduler) {
        List<Node> activity_list = project.nodes.stream().sorted(Comparator.comparing(n -> n.duration)).collect(Collectors.toList());
