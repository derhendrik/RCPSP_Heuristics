...
public class MaxCUMRED extends AbstractPriorityRule {
    ...
    public List<Node> generate_activity_list(Project project, Scheduler scheduler) {
        List<Node> activity_list = project.nodes.stream().sorted(Comparator.comparing(n -> n.cumred)).collect(Collectors.toList());
